...
n_burn<-3000
keep_every<-2
iters_to_keep<-seq(from=3001, to=5000, by=2)
...
XT<-rbind(XT,XT,XT,XT,XT,XT,XT,XT,XT,XT)

XT[,"BCBGMRS"]<-rep(seq(min(X[,"BCBGMRS"]), max(X[,"BCBGMRS"]), length.out=10), each=4118)
XT[,"BCBGSRS"]<-rep(seq(min(X[,"BCBGSRS"]), max(X[,"BCBGSRS"]), length.out=10), each=4118)
...
save(my_mod, file = "~/YOUR_DIRECTORY/Hungry.RData", compress = "xz")
...
write.table(tempdf,"~/YOUR_DIRECTORY/Hungry_ATES.csv", row.names = F, append=T, sep = ",", col.names=F)
