...
maths_vars<-c("BSDAGE", "BSBG01", "BSBG03", "BSBG04", "BSBG07",
              ...
              "BSBG05A", "BSBG10", "BSBG05C", "BSBG05D", "BSBG05E", "BSBG05F", "BSBG05G", 
              "BSBG11A", "BSBG05B",
...
Z1<-ifelse(XY$BSBG11B.x>=2, 1, 0)

Z2<-ifelse(XY$BSBG11B.y>=2, 1, 0)
...
#Set random seed for splitting folds
set.seed(123)
num_folds<-10
...
for(fold in 1:num_folds)
{
  ...
  #mvbcf applied to timss as univariate model
  my_mod <- fast_bart(X_train, 
                      as.matrix(Y_train[,1]), 
                      as.matrix(Z_train[,1]), 
                      X2_train,
  ...
  bcf_y_preds_1<-rowMeans(my_mod$predictions_test[,1,-c(1:n_burn)])+Z_test[,1]*rowMeans(my_mod$predictions_tau_test[,1,-c(1:n_burn)])
  ...
  bcf_y_preds_2<-rowMeans(my_mod$predictions_test[,1,-c(1:n_burn)])+Z_test[,1]*rowMeans(my_mod$predictions_tau_test[,1,-c(1:n_burn)])
...
data <- data.frame(
  mean(my_rmse_1_list),
  mean(my_rmse_2_list),
  mean(bcf_rmse_1_list),
  mean(bcf_rmse_2_list),
  mean(bart_rmse_1_list),
  mean(bart_rmse_2_list)
)

save(data, file = "Hungry_Data.RData", compress = "xz")
